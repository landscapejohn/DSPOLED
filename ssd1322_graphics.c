...
#define LIBSSD1322_HAVE_FREETYPE2
#define LIBSSD1322_HAVE_DECL_STRERROR_R

#ifdef LIBSSD1322_HAVE_DECL_STRERROR_R
 // do nothing
#else
 // rewrite it
...
#endif
#ifdef LIBSSD1322_HAVE_FREETYPE2
#include <ft2build.h>
#include FT_FREETYPE_H
...
#ifndef SSD1322_FB_BAD_PTR_RETURN
#define SSD1322_FB_BAD_PTR_RETURN(P,RC) do { \
  if (!(P != NULL && (P)->buffer != NULL && (P)->len > 0)) { \
    return RC; \
  } \
} while (0)
#endif // SSD1322_FB_BAD_PTR_RETURN
#ifndef SSD1322_FB_GET_ERRFP
#define SSD1322_FB_GET_ERRFP(P) ((P) != NULL && (P)->err != NULL && (P)->err->err_fp != NULL) ? (P)->err->err_fp : stderr;
#endif
#ifndef SSD1322_ERR_GET_ERRFP
#define SSD1322_ERR_GET_ERRFP(P) ((P) != NULL && (P)->err_fp != NULL) ? (P)->err_fp : stderr;
#endif

#ifdef LIBSSD1322_HAVE_PTHREAD
#include <pthread.h>
typedef pthread_mutex_t ssd1322_lock_t;
#define SSD1322_LOCK(A) pthread_mutex_lock((A))
#define SSD1322_UNLOCK(A) pthread_mutex_unlock((A))
#define SSD1322_LOCK_DESTROY(A) pthread_mutex_destroy((A))
#define SSD1322_LOCK_CREATE(A,B) \
    do { \
...
#else
typedef void *ssd1322_lock_t;
#define SSD1322_LOCK(A) do{} while(0)
#define SSD1322_UNLOCK(A) do{} while(0)
#define SSD1322_LOCK_DESTROY(A) do{} while(0)
...
static ssd1322_font_t *ssd1322_font_create(ssd1322_err_t *err)
{
    ...
    if (font) {
        SSD1322_LOCK_CREATE(&(font->_lock), true);// allow recursive lock
        FT_Error ferr = 0;
        do {
            ...
            for (uint32_t idx = SSD1322_FONT_DEFAULT; idx < SSD1322_FONT_MAX; ++idx) {
                font->faces[idx] = NULL;
                ferr = FT_New_Face(font->lib, ssd1322_fontface_paths[idx], 0,
                    &(font->faces[idx]));
                if (ferr) {
                    fprintf(err_fp, "ERROR: FreeType FT_New_Face(%s => %s) error: %d (%s)\n",
                        ssd1322_fontface_names[idx], ssd1322_fontface_paths[idx],
                        ferr, FT_Error_String(ferr));
                    break;
                }
            }
...
static void ssd1322_font_destroy(ssd1322_font_t *font, ssd1322_err_t *err)
{
    if (font) {
        FILE *err_fp = SSD1322_ERR_GET_ERRFP(err);
        if (font->lib) {
            SSD1322_LOCK(&(font->_lock));
            FT_Error ferr = 0;
            for (uint32_t idx = 0; idx < SSD1322_FONT_MAX; ++idx) {
                if (font->faces[idx]) {
                    ferr = FT_Done_Face(font->faces[idx]);
                    if (ferr) {
                        fprintf(err_fp, "WARN: Freetype FT_Done_Face(%s) error: %d (%s)\n",
                            ssd1322_fontface_paths[idx], ferr, FT_Error_String(ferr));
                    }
                    font->faces[idx] = NULL;
                }
            }
            ...
            font->lib = NULL;
            SSD1322_UNLOCK(&(font->_lock));
        }
        SSD1322_LOCK_DESTROY(&(font->_lock));
...
static int ssd1322_font_render_string(ssd1322_framebuffer_t *fbp,
    const char *font_file, ssd1322_fontface_t font_idx, uint8_t font_size,
    ...
    if (fbp && fbp->font && (font_idx < SSD1322_FONT_MAX || font_file) && str && slen > 0) {
        ...
        do {
            ...
            if (font_idx < SSD1322_FONT_MAX) {
                ...
                if (ferr) {
                    fprintf(err_fp, "ERROR: FreeType FT_New_Face(%s => %s) error: %d (%s)\n",
                        ssd1322_fontface_names[SSD1322_FONT_CUSTOM], font_file,
                        ferr, FT_Error_String(ferr));
                    break;
                }
                free_the_face = true;
            ...
            if (face) {
                ...
                if (ferr) {
                    fprintf(err_fp, "ERROR: FreeType FT_Set_Char_Size(%s, %d) error: %d (%s)\n",
                        ssd1322_fontface_paths[font_idx], font_size, ferr, FT_Error_String(ferr));
                    rc = -1;
                    break;
                }
                if (rotation_degrees) {
                ...
                for (size_t idx = 0; idx < slen; ++idx) {
                    ...
                    if (0) {
                        ...
                        if (ferr) {
                            fprintf(err_fp, "WARN: Freetype FT_Load_Char(0x%lx) error: %d (%s)\n",
                ...
                fprintf(err_fp, "ERROR: Font %s does not have a face pointer\n",
                    ssd1322_fontface_names[font_idx]);
                rc = -1;
                break;
            }
        } while (0);
...
ssd1322_err_t *ssd1322_err_create(FILE *fp)
{
    if (fp == NULL)
        fp = stderr;
    ssd1322_err_t *err = calloc(1, sizeof(ssd1322_err_t));
    if (!err) {
        fprintf(fp, "ERROR: Out of memory allocating %zu bytes\n", sizeof(ssd1322_err_t));
        return NULL;
    }
    err->err_fp = fp;
    err->errnum = 0;
    ...
    SSD1322_ATOMIC_ZERO(&(err->_ref));
    SSD1322_ATOMIC_INCREMENT(&(err->_ref));
    return err;
}
...
void ssd1322_err_destroy(ssd1322_err_t *err)
{
    if (err) {
        int zero = 0;
        SSD1322_ATOMIC_DECREMENT(&(err->_ref));
        if (SSD1322_ATOMIC_IS_EQUAL(&(err->_ref), &zero)) {
            if (err->errbuf) {
                free(err->errbuf);
                err->errbuf = NULL;
            }
...
ssd1322_framebuffer_t *ssd1322_framebuffer_create(uint16_t width, uint16_t height, ssd1322_err_t *err)
{
    FILE *err_fp = SSD1322_ERR_GET_ERRFP(err);
    if (width == 0 || height == 0) {
        fprintf(err_fp, "ERROR: Width: %d Height: %d cannot be zero\n", width, height);
        return NULL;
    }
    ssd1322_framebuffer_t *fbp = calloc(1, sizeof(ssd1322_framebuffer_t));
    if (!fbp) {
    ...
    do {
        ...
        fbp->height = height;
        fbp->err = err;
        SSD1322_ERR_REF_INC(err);
        fbp->len = sizeof(uint8_t) * (fbp->width * fbp->height / 2);
        ...
        fbp->font = ssd1322_font_create(fbp->err);
        if (!fbp->font) {
...
void ssd1322_framebuffer_destroy(ssd1322_framebuffer_t *fbp)
{
    if (fbp) {
        if (fbp->font) {
            ssd1322_font_destroy(fbp->font, fbp->err);
            fbp->font = NULL;
        }
        ssd1322_err_destroy(fbp->err);
        fbp->err = NULL;
        if (fbp->buffer) {
            free(fbp->buffer);
            fbp->buffer = NULL;
...
int ssd1322_framebuffer_bitdump_custom(ssd1322_framebuffer_t *fbp, char zerobit, char onebit, bool use_space, bool use_color)
{
    ...
    for (size_t y = 0; y < fbp->height; ++y) {
        fprintf(err_fp, "%04zX ", y);
        for (size_t x = 0; x < fbp->width; ++x) {
            int8_t pixel = ssd1322_framebuffer_get_pixel(fbp, x, y);
            if (pixel) {// red
...
static void ssd1322_framebuffer_draw_text_options_handler(
    const ssd1322_graphics_options_t *opts, size_t num_opts,
            ...
            switch (opts[i].type) {
                ...
                case SSD1322_OPT_ROTATE_PIXEL:
                    if (opts[i].value.rotation_degrees % 90 == 0) {
                        ...
                        fprintf(err_fp, "WARN: SSD1322_OPT_ROTATE_PIXEL only accepts rotation_degrees in multiples of 90\n");
                        rotate_pixel = 0;
                    }
                    break;
                default:
...
ssize_t ssd1322_framebuffer_draw_text_extra(ssd1322_framebuffer_t *fbp,
    ...
    uint8_t x, uint8_t y, ssd1322_fontface_t fontface,
    uint8_t font_size,
    const ssd1322_graphics_options_t *opts, size_t num_opts,
    ssd1322_framebuffer_box_t *bbox)
{
    return ssd1322_framebuffer_draw_text_utf8(fbp, (const uint8_t *)str, slen, x, y, fontface,
    ...
    ssd1322_framebuffer_box_t *bbox)
{
    ...
    if (fbp && str) {
        ...
        int rc = ssd1322_framebuffer_draw_text_utf32(fbp, str32, slen32,
    ...
    ssd1322_framebuffer_box_t *bbox)
{
    ...
    if (fbp && str) {
        ...
        uint8_t rotate_pixel = 0;
        int16_t rotation_degrees = 0;
        ssd1322_framebuffer_draw_text_options_handler(opts, num_opts, &font_file,
            &rotate_pixel, &rotation_degrees, err_fp);
        if (fontface >= SSD1322_FONT_CUSTOM) {
            if (font_file != NULL) {
                return ssd1322_font_render_string(fbp,
                    font_file, SSD1322_FONT_CUSTOM, font_size,
                    str, slen, sizeof(uint32_t), (uint16_t)x, (uint16_t)y,
                    rotation_degrees, rotate_pixel, bbox);
            }
            else {
                fprintf(err_fp, "ERROR: If using %s then you need to use SSD1322_OPT_FONT_FILE\n",
                    ssd1322_fontface_names[SSD1322_FONT_CUSTOM]);
                return -1;
            }
        }
        else {
...
ssize_t ssd1322_framebuffer_draw_text_extra(ssd1322_framebuffer_t *fbp,
    const char *str, size_t slen,
    uint16_t x, uint16_t y, ssd1322_fontface_t fontface,
    uint8_t font_size,
    const ssd1322_graphics_options_t *opts, size_t num_opts,
    ssd1322_framebuffer_box_t *bbox)
{
...
#endif /* LIBSSD1322_HAVE_UNISTR_H */

ssize_t ssd1322_framebuffer_draw_text(ssd1322_framebuffer_t *fbp,
    const char *str, size_t slen,
    uint16_t x, uint16_t y, ssd1322_fontface_t fontface,
    uint8_t font_size, ssd1322_framebuffer_box_t *bbox)
{
    if (fontface < SSD1322_FONT_CUSTOM) {
        return ssd1322_framebuffer_draw_text_extra(fbp, str, slen, x, y,
            fontface, font_size, NULL, 0, bbox);
    }
    else {
        FILE *err_fp = SSD1322_FB_GET_ERRFP(fbp);
        fprintf(err_fp, "ERROR: Fontface cannot be %s in %s(). use %s_extra()\n",
            ssd1322_fontface_names[(size_t)fontface], __func__, __func__);
        return -1;
    }
}
    ...
    uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, bool color)
{
    ...
    if (delta_x == 0) {
        if (delta_y != 0) {
            ...
            ssd1322_framebuffer_put_pixel(fbp, x0, y0, color);
            while (delta_y--) {
                ssd1322_framebuffer_put_pixel(fbp, x0, y0, color);
                y0++;
            }
