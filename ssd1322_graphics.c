...
// Font Rendering
/// <summary>
/// Load defined fonts into memory
/// </summary>
...
static ssd1322_font_t* ssd1322_font_create(ssd1322_err_t * err)
{
	...
	if (font)
	{
		...
		do
		{
			...
			{
				font->faces[idx] = NULL;
				ferr = FT_New_Face(font->lib, ssd1322_fontface_paths[idx], 0, &(font->faces[idx]));
				if (ferr)
				{
					fprintf(err_fp, "ERROR: FreeType FT_New_Face(%s => %s) error: %d (%s)\n", ssd1322_fontface_names[idx], ssd1322_fontface_paths[idx], ferr, FT_Error_String(ferr));
					break;
				}
			}
...
static int ssd1322_font_render_string(ssd1322_framebuffer_t * fbp, const char* font_file, ssd1322_fontface_t font_idx, uint8_t font_size, const void* str, size_t slen, size_t chsz, uint16_t x, uint16_t y, int16_t rotation_degrees, uint8_t rotate_pixel, ssd1322_framebuffer_box_t * bbox)
{
	...
	if (fbp && fbp->font && (font_idx < SSD1322_FONT_MAX || font_file) && str && slen > 0)
	{
		...
		do
		{
			...
			{
				// Use one of the pre-defined fonts
				face = font->faces[font_idx];
				free_the_face = false;
			}
			else if (font_file)
			{
				// Use the font file specified in the font_file argument
				...
				if (ferr)
				{
					fprintf(err_fp, "ERROR: FreeType FT_New_Face(%s => %s) error: %d (%s)\n", ssd1322_fontface_names[SSD1322_FONT_CUSTOM], font_file, ferr, FT_Error_String(ferr));
					break;
				}
				free_the_face = true;
			}
...
ssize_t ssd1322_framebuffer_draw_text_extra(ssd1322_framebuffer_t * fbp, const char* str, size_t slen, uint16_t x, uint16_t y, ssd1322_fontface_t fontface, uint8_t font_size, const ssd1322_graphics_options_t * opts, size_t num_opts, ssd1322_framebuffer_box_t * bbox)
{
	...
	if (fbp && str)
	{
		...
		uint8_t rotate_pixel = 0;
		int16_t rotation_degrees = 0;
		ssd1322_framebuffer_draw_text_options_handler(opts, num_opts, &font_file, &rotate_pixel, &rotation_degrees, err_fp);

		if (fontface >= SSD1322_FONT_CUSTOM)
		{
			if (font_file != NULL)
			{
				return ssd1322_font_render_string(fbp, font_file, SSD1322_FONT_CUSTOM, font_size, str, slen, sizeof(char), (uint16_t)x, (uint16_t)y, rotation_degrees, rotate_pixel, bbox);
			}
			else
			{
		...
		{
			return ssd1322_font_render_string(fbp, NULL, fontface, font_size, str, slen, sizeof(char), (uint16_t)x, (uint16_t)y, rotation_degrees, rotate_pixel, bbox);
		}
	}
...
ssize_t ssd1322_framebuffer_draw_text(ssd1322_framebuffer_t * fbp, const char* str, size_t slen, uint16_t x, uint16_t y, ssd1322_fontface_t fontface, uint8_t font_size, ssd1322_framebuffer_box_t * bbox)
{
	if (fontface < SSD1322_FONT_CUSTOM)
	{
		return ssd1322_framebuffer_draw_text_extra(fbp, str, slen, x, y, fontface, font_size, NULL, 0, bbox);
	}
	else
	{
		FILE* err_fp = SSD1322_FB_GET_ERRFP(fbp);
